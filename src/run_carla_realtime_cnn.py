...
from harsh_event_model import RealtimeCNNHarshEventDetector
...
def _packet_from_imu(
    ...
    return {
        ...
        # Subtract gravity (9.81 m/s^2) from local Z accelerometer to match training data
        "acc_z": float(data.accelerometer.z) - 9.81,
        # Gyroscope values in the training set are in radians/sec, NOT degrees/sec
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Realtime 1D-CNN harsh-event detection from CARLA IMU stream")
    ...
    def _on_imu(data: carla.IMUMeasurement) -> None:
        ...
        # Print state updates only on harsh events OR if print_safe is explicitly requested
        if event != "safe" or args.print_safe:
            ts = f"{data.timestamp:.3f}"
            print(
                f"t={ts}s model={raw_event}->{event} conf={conf:.3f} "
                f"acc=({packet['acc_x']:.2f},{packet['acc_y']:.2f},{packet['acc_z']:.2f}) "
                f"gyro=({packet['gyro_x']:.2f},{packet['gyro_y']:.2f},{packet['gyro_z']:.2f})"
            )
